...
**🆕 תכונה חדשה:** דיקור נוכחות NFC! השתמש ב-`?nfc=true` לדיקור כניסה/יציאה אוטומטי.
...
- 🆕 **דיקור נוכחות NFC** (עם iOS Shortcuts)
...
האפליקציה תומכת בדיקור כניסה/יציאה אוטומטי באמצעות תגי NFC ו-iOS Shortcuts!
...
2. **צור קיצור חדש:**
   - לחץ על **"+"**
   - הוסף פעולה: **Open URL**
   - **URL:** הוסף את הקישור שלך + `?nfc=true`
   ```
   https://shopping-1zj2f3e5o-gvc00771-9581s-projects.vercel.app?nfc=true
   ```
   - ⚠️ **חשוב:** הפרמטר `?nfc=true` הוא חובה!

3. **חבר לתג NFC:**
   - **Shortcuts** → **Automation** → **"+"**
   - בחר **NFC**
   - **Scan** - סרוק את התג שלך
   - **Run Shortcut** → בחר את הקיצור שיצרת
   - **כבה** את "Ask Before Running"

### מה קורה כשמקרבים את הטלפון לתג?

1. ✅ **אוטומציה מופעלת** - iOS Shortcuts מזהה את התג
2. 🌐 **דפדפן נפתח** - Safari פותח את הקישור
3. ⚡ **דיקור מיידי** - המערכת מזהה `?nfc=true`
4. 🎉 **מסך מלא** - הצגת כניסה/יציאה עם אימוג'י ושעה
5. ⏰ **סגירה אוטומטית** - אחרי 5 שניות (במובייל)
...
כשמבצעים דיקור, מוצג מסך מלא:
- 🎨 **צבע רקע:** ירוק (כניסה) / אדום (יציאה)
- 😊 **אימוג'י גדול:** ✅ (כניסה) / 👋 (יציאה)
- ⏰ **שעה מדויקת**
- 📅 **תאריך**
- 🔘 **כפתור סגירה**
...
- **NTAG213** (180 bytes) - ~$0.50 ⭐ מומלץ!
- **NTAG215** (540 bytes) - ~$1.00
- **NTAG216** (930 bytes) - ~$1.50

### טיפים

💡 **למובייל:** הדיקור יסגר אוטומטית אחרי 5 שניות  
💡 **למחשב:** לחץ על כפתור "סגור" לסגירה ידנית  
💡 **פרטיות:** כל הנתונים נשמרים רק במכשיר שלך (localStorage)  
💡 **היסטוריה:** הנתונים נשמרים ב-`nfc_attendance_records`
